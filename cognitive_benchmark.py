...
improves retrieval in ways that plain ANN cannot. It runs three scenarios
...
Usage:
    python cognitive_benchmark.py
    python cognitive_benchmark.py --dimension 64 --verbose
"""
...
def scenario_abstraction_traversal(tsm, dim):
    """
    ...
    - 15 "distractor" memories with various concepts and vectors scattered
      around the query vector. ANN returns these before the target.
    ...
    With 25 memories and top_k=5, ANN returns the 5 closest vectors (rust-
    only + some distractors). The target is NOT in the ANN top-5 (it has
    near-zero cosine to the query). The cognitive search should find it
    through the abstraction path: concept:rust → parent → concept:safety →
    target, and the fusion should rank it in the top-5.
    """
    logger.info("Scenario 1: Abstraction traversal")
    ...
    # Query: vector close to rust cluster, text "rust", top_k=5.
    query_vec = make_close_vec(rust_cluster, 0.1, 777)

    ann_results = tsm.search_ann(query_vec, top_k=5)
    cog_results = tsm.search("rust", query_vec, top_k=5)
...
def scenario_refinement_surfacing(tsm, dim):
    """
    ...
    """
    logger.info("Scenario 2: Refinement surfacing")
...
def scenario_reinforcement_boosting(tsm, dim):
    """
    ...
    by cosine only.
    """
    logger.info("Scenario 3: Reinforcement boosting")
...
def scenario_contradiction_surfacing(tsm, dim):
    """
    ...
    """
    logger.info("Scenario 4: Contradiction surfacing")
...
def run_scenario(tsm_module, dim, config_overrides, scenario_fn):
...
def main():
    ...
    cog1 = run_scenario(tsm, dim, {
        ...
        "spreading_decay": 0.7,
        "spreading_beta": 0.0,
    }, scenario_abstraction_traversal)
    ...
    }, scenario_reinforcement_boosting)
    ...
    cog4 = run_scenario(tsm, dim, {
        ...
        "spreading_iterations": 6,
        "spreading_decay": 0.7,
    }, scenario_contradiction_surfacing)
